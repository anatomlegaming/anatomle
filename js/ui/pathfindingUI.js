...
function PathfindingUI({ gameTitle, accentColor, state, handlers }) {
    ...
    const NAV_H = 58, BAR_H = 46, SRCH_H = 68, PANEL_W = 420, AD_W = 160;
    ...
    return (
        ...
        {/* CHALLENGE BAR */}
        <div style={{position:'fixed',top:NAV_H,left:0,right:PANEL_W+AD_W,height:BAR_H,zIndex:100,
        ...
        {/* RIGHT PANEL */}
        <div style={{position:'fixed',top:NAV_H,right:AD_W,width:PANEL_W,bottom:0,zIndex:100,
            ...
            <div style={{padding:12,flexShrink:0}}>
                <button onClick={phase === 'playing' ? (onGiveUp || onNewGame) : onNewGame} style={{width:'100%',padding:'11px 0',background:'white',color:'black',border:'none',fontFamily:'EB Garamond,serif',fontWeight:900,fontSize:10,textTransform:'uppercase',letterSpacing:'0.2em',cursor:'pointer'}}>
        ...
        {/* SEARCH BAR */}
        <div style={{position:'fixed',bottom:0,left:0,right:PANEL_W+AD_W,height:SRCH_H,zIndex:150,
            background:'rgba(10,15,30,0.98)',borderTop:'1px solid #1e293b',padding:'10px 14px',display:'flex',alignItems:'center'}}>
            <div style={{flex:1,position:'relative'}}>
                {suggestions.length > 0 && (
                    <div style={{position:'absolute',bottom:'100%',left:0,right:0,background:'#fdf6ec',border:'1px solid #e8d5c0',borderBottom:'none',zIndex:300,maxHeight:280,overflowY:'auto'}}>
                        {suggestions.map((s,i) => (
